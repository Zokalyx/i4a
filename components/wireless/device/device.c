...
void device_init(DevicePtr device_ptr, const char *device_uuid,
		 ...
		 uint8_t ap_max_sta_connections, uint8_t device_is_root,
		 Device_Mode mode)
{
	...
	if (mode == AP) {
		device_init_ap(device_ptr, ap_channel_to_emit,
			       wifi_network_prefix, device_uuid,
			       wifi_network_password, ap_max_sta_connections);
	} else if (mode == STATION) {
		device_init_station(device_ptr, wifi_network_prefix,
				    device_orientation, device_uuid,
		    ...
		    const char *password, uint8_t max_sta_connections)
{
	...
	ap_init(device_ptr->access_point_ptr, channel, wifi_ssid, password,
		max_sta_connections);
};
...
void device_init_station(DevicePtr device_ptr, const char *wifi_ssid_like,
			 ...
			 const char *password)
{
	station_init(device_ptr->station_ptr, wifi_ssid_like, orientation,
...
{
	ap_set_network(device_ptr->access_point_ptr, network_cidr,
		       network_gateway, network_mask);
};
...
static void device_connect_station_task(void *arg)
{
	DevicePtr device_ptr =
		(DevicePtr)arg; // Get the device pointer from the task argument
	...
	while (is_on_connect_loop) {
		...
		if (!station_is_active(device_ptr->station_ptr)) {
			ESP_LOGI(
				LOGGING_TAG,
				"Wi-Fi not connected. Scanning for available networks...");
			station_find_ap(device_ptr->station_ptr);

			if (station_found_ap(device_ptr->station_ptr)) {
				ESP_LOGI(LOGGING_TAG,
					 "Wi-Fi found! Attempting to connect.");
				station_connect(device_ptr->station_ptr);
			} else {
				ESP_LOGE(
					LOGGING_TAG,
					"No Wi-Fi found. Re-scanning in 10 seconds.");
			}

			// Wait before trying again
			vTaskDelay(
				pdMS_TO_TICKS(10000)); // Scan every 10 seconds
		}

		vTaskDelay(pdMS_TO_TICKS(
			10000)); // Wait 10 seconds before checking if station is not currently trying to connect
	}
...
void device_connect_station(DevicePtr device_ptr)
{
	...
	xTaskCreate(device_connect_station_task, "device_connect_station_task",
		    4096, device_ptr, 5, NULL);
}
...
{
	device_init(device, device_uuid, device_orientation, wifi_ssid_prefix,
		    wifi_password, ap_channel_to_emit, ap_max_sta_connections,
		    device_is_root, mode);

	if (mode == AP) {
		device_start_ap(device);
	} else if (mode == STATION) {
		device_start_station(device);
		device_connect_station(device);
	}
}
