...
// Waits for the center device to share its MAC through the ring link
void config_setup(void)
{
	...
	memset(s_config.center_mac, 0, sizeof(s_config.center_mac));
	// Wait for the center device to share its MAC through the ring link
	uint8_t center_mac_received = 0;
	while (!center_mac_received) {
	}
