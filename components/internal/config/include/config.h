...
#define CONFIG_PIN_MASK                                    \
	((1ULL << CONFIG_PIN_0) | (1ULL << CONFIG_PIN_1) | \
	 (1ULL << CONFIG_PIN_2))
...
typedef struct {
	...
	uint32_t rx_ip_addr;
	uint32_t tx_ip_addr;
	// All ESP32s in a node should share the same UUID in the SSID.
	// We can use the MAC of the center device as the UUID.
	uint8_t center_mac[6];
} config_t;
