...
static const struct esp_netif_netstack_config netif_netstack_config = {
	.lwip = { .init_fn = ring_link_rx_netstack_lwip_init_fn,
...
esp_err_t ring_link_rx_netif_receive(ring_link_payload_t *p)
{
	...
	ESP_LOGI(TAG, "SRC IP: %d.%d.%d.%d", octet1_, octet2_,
		 (int)((addr_ >> 8) & 0xFF), (int)(addr_ & 0xFF));
	...
	// Print IP address for debugging
	ESP_LOGI(TAG, "Destination IP: %d.%d.%d.%d", octet1, octet2,
	...
	if (iphdr_len > p->len) {
		ESP_LOGW(
			TAG,
			"Payload size (%d) is smaller than IP length (%d). Discarding packet.",
			p->len, iphdr_len);
		return ESP_OK;
	}
...
static err_t output_function(struct netif *netif, struct pbuf *p,
...
static err_t linkoutput_function(struct netif *netif, struct pbuf *p)
{
	...
	if (!esp_netif) {
		LWIP_DEBUGF(
			NETIF_DEBUG,
			("corresponding esp-netif is NULL: netif=%p pbuf=%p len=%d\n",
			 netif, p, p->len));
		return ERR_IF;
	}
	if (q->next == NULL) {
		...
		ESP_LOGE(
			TAG,
			"low_level_output: pbuf is a list, application may has bug");
...
esp_netif_recv_ret_t ring_link_rx_netstack_lwip_input_fn(void *h, void *buffer,
							 size_t len,
							 void *l2_buff)
{
...
static esp_err_t ring_link_rx_driver_transmit(void *h, void *buffer, size_t len)
{
	ESP_LOGW(
		TAG,
		"This netif should not transmit but ring_link_rx_driver_transmit was called");
	return ESP_OK;
}

static esp_err_t esp_netif_ring_link_driver_transmit(void *h, void *buffer,
						     size_t len)
{
...
static esp_err_t ring_link_rx_driver_post_attach(esp_netif_t *esp_netif,
						 void *args)
{
	ESP_LOGI(
		TAG,
		"Calling esp_netif_ring_link_post_attach_start(esp_netif_t * esp_netif, void *args)");
...
static void ring_link_rx_default_handler(void *arg, esp_event_base_t base,
...
static void ring_link_rx_default_action_start(void *arg, esp_event_base_t base,
					      int32_t event_id, void *data)
{
...
void ring_link_set_config_mac(void *arg, esp_event_base_t base,
			      int32_t event_id, void *data)
{
	ESP_LOGI(TAG, "Calling ring_link_set_config_mac");

	// Receive the center device MAC address
}
...
esp_err_t ring_link_rx_netif_init(void)
{
	...
	ESP_ERROR_CHECK(ring_link_netif_esp_netif_attach(
		ring_link_rx_netif, ring_link_rx_driver_post_attach));

	ESP_ERROR_CHECK(esp_event_handler_instance_register(
		RING_LINK_RX_EVENT, RING_LINK_EVENT_START,
		ring_link_rx_default_action_start, NULL, NULL));
	ESP_ERROR_CHECK(esp_event_post(RING_LINK_RX_EVENT,
				       RING_LINK_EVENT_START, NULL, 0,
				       portMAX_DELAY));

	ESP_ERROR_CHECK(esp_event_handler_instance_register(
		RING_LINK_RX_EVENT, RING_LINK_MAC_ADDRESS,
		ring_link_set_config_mac, NULL, NULL));
